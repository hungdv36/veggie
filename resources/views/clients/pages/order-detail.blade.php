...
<div class="container my-5">
    <div class="card shadow-lg border-0 rounded-4">
        <div class="card-body p-4">
                ...
                <table class="table align-middle table-hover">
                    ...
                    <tbody>
                        @foreach ($order->orderItems as $item)
                            <tr>
                                <td><img src="{{ asset('storage/' . $item->product->image_url) }}" width="60" class="rounded"></td>
            ...
            <!-- Hành động -->
            @if ($order->status == 'pending')
                <form action="{{ route('order.cancel', $order->id) }}" method="POST" class="mt-4" onsubmit="return confirm('Bạn có chắc chắn muốn hủy đơn hàng này?');">
                    @csrf
                            ...
                            <tbody>
                                @foreach ($order->orderItems as $item)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('product.detail', $item->product->slug) }}" class="btn btn-primary btn-sm">
